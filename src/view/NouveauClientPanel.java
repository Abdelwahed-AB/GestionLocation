...
public class NouveauClientPanel extends JPanel {
	...
	// on a besion de ces variables ï¿½ l'interieur de plusieurs fonction donc on les
	// rendre globals
	...
	public NouveauClientPanel(JPanel panel, JTable table) {
		...
		buttonEffacer.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				// effacer tous les informations pour les redï¿½finir
		...
		imageButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// ouvrir une fenetre pour sï¿½lï¿½ctionnï¿½ l'image
				JFileChooser chooser = new JFileChooser();
				chooser.setDialogTitle("Choisir une image");
				//ajouter un filtre ï¿½ la fenetre de choix
				FileNameExtensionFilter filter = new FileNameExtensionFilter("*.Image", "jpg", "png");
				chooser.addChoosableFileFilter(filter);
				chooser.showOpenDialog(null);
				// si l'utisitaeur ne sï¿½lï¿½ctionne aucune image
				File file = chooser.getSelectedFile();
				if (file != null) {
					//si l'utilisateur sï¿½lectionne un fichier d'autre type
		...
		buttonSauvgarder.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (!nomTextField.getText().isBlank() && !prenomTextField.getText().isBlank()
					...
					if (nomTextField.getText().matches("^[a-zA-Z _'ï¿½-]*")) {
						if (prenomTextField.getText().matches("^[a-zA-Z _'ï¿½-]*")) {
							if (teleTextField.getText().matches("0[0-9]*|212[0-9]*")) {
								...
								//si le client est bien crï¿½er dans la base de donnï¿½e afficher message "Opï¿½ration Effectuï¿½e avce Succï¿½e"
								if (b) {
									JOptionPane.showConfirmDialog(null, "Operation Effectee avce Succee", "Succï¿½e",
